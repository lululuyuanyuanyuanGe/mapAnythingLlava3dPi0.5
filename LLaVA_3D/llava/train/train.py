...
#    Copyright 2023 Rohan Taori, Ishaan Gulrajani, Tianyi Zhang, Yann Dubois, Iuechen Li
#
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):  # 对于 pretraining 来说我们只需要保存 projection layer 
        # Only save Adapter
        keys_to_match = ['mm_projector', 'video_tower']
...
def preprocess_target(
    ...
    if not is_multimodal:
        return sources
    box_processor = data_args.box_processor
    ...
    for idx, source in enumerate(sources):
        ...
        for sentence in source:
            ...
            if boxes_seq is not None:
                # map box seq
                boxes_seq = map_obj(boxes, boxes_seq)
                # reformat; replace <boxes> placeholder
                converted = box_processor(words, boxes_seq)
                words = converted
            points_seq: List[List[int]] = sentence.get('points_seq', None)
            if points_seq is not None:
                # map point seq
                points_seq = map_obj(points, points_seq)
                # reformat; replace <points> placeholder
                converted = box_processor.call_on_point(words, points_seq)
                words = converted
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            video_file = self.list_data_dict[i]['video']  # could be scene name or image name
            ...
            if isinstance(video_file, str):
                ...
                video = [os.path.join(video_folder, frame_file) for frame_file in video_file]  # video_path
            else:
                raise NotImplementedError
            video_dict = processor.preprocess(video, return_tensors='pt')
            image = video_dict['images']
        ...
        if 'image' in self.list_data_dict[i] or 'video' in self.list_data_dict[i]:
            data_dict['image'] = image  # (3, H, W) or (V, 3, H, W)
        elif self.data_args.is_multimodal:
            # image does not exist in the data, but the model is multimodal
            crop_size = self.data_args.image_processor.crop_size
            data_dict['image'] = torch.zeros(3, crop_size['height'], crop_size['width'])

        # add video data into data_dict
        if 'video' in self.list_data_dict[i]:
            data_dict['depth'] = video_dict['depth_images']  # （V, H, W)
            data_dict['pose'] = video_dict['poses']  # (V, 4, 4)
            data_dict['intrinsic'] = video_dict['intrinsic']  # (4, 4)
