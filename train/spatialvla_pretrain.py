...
def main():
    ...
    if model_args.model_name_or_path is not None:
        ...
        model = SpatialVLAForConditionalGeneration(
            config,
            vision_model=paligemma_model.vision_tower, #  SigLIP-So400m/14
            vision_zoe_model=vision_zoe_model, # 不需要
            projector_model=paligemma_model.multi_modal_projector, # Linear model
            language_model=gemma2,
        )
        model = SpatialVLAForConditionalGeneration.from_pretrained(
            config=config,
            vision_moddel=siglip_model,
            torch_dtype=torch_dtype,
            local_files_only=True,
        )
